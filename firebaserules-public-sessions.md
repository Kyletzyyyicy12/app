...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    function isOrganizerOrTeacher() {
      return isOrganizer() || isTeacher() || isAdmin();
    }
    ...
    match /users/{userId} {
      ...
      // Organizers can read participant profiles (but not other organizers/admins)
      allow read: if isOrganizer() &&
                  exists(/databases/$(database)/documents/users/$(userId)) &&
                  get(/databases/$(database)/documents/users/$(userId)).data.role == 'participant';
      
      // Participants can see basic info of organizers who invited them
      allow read: if isAuthenticated() && 
                  getUserRole() == 'participant' &&
                  exists(/databases/$(database)/documents/liveInvitations/$(userId)) &&
                  get(/databases/$(database)/documents/liveInvitations/$(userId)).data.studentId == request.auth.uid;
    }
    ...
    match /wheels/{wheelId} {
      ...
      match /viewers/{viewerId} {
        allow read: if true;
        // Allow unauthenticated users to join live sessions
        allow create: if resource.data.isActive == true;
        allow update: if resource.data.isActive == true;
        allow delete: if resource.data.isActive == true;
      }
    }
    ...
    match /wheelSpins/{spinId} {
      allow read: if isAuthenticated();
      allow create: if isOrganizerOrTeacher();
      allow update: if isAuthenticated();
      allow delete: if isOrganizerOrTeacher();
    }
    ...
    match /liveDrawSessions/{sessionId} {
      // Read allowed for anyone (public access to active sessions)
      allow read: if resource.data.isActive == true;
      
      // Create by organizer/teacher/admin
      allow create: if isOrganizerOrTeacher();
      
      // Updates for specific fields
      allow update: if (isAuthenticated() && 
      ...
      match /viewers/{viewerId} {
        allow read: if true;
        // Allow unauthenticated users to join active sessions
        allow create: if get(/databases/$(database)/documents/liveDrawSessions/$(sessionId)).data.isActive == true;
        allow update: if get(/databases/$(database)/documents/liveDrawSessions/$(sessionId)).data.isActive == true;
      ...
      match /reactions/{reactionId} {
        allow read: if true;
        // Allow unauthenticated users to create reactions in active sessions
        allow create: if get(/databases/$(database)/documents/liveDrawSessions/$(sessionId)).data.isActive == true;
      ...
      match /drawData/{dataId} {
        allow read: if isAuthenticated();
        allow write: if isOrganizerOrTeacher();
      }
      ...
      match /spinResults/{resultId} {
        allow read: if isAuthenticated();
        allow create: if isAuthenticated();
        allow update, delete: if isOrganizerOrTeacher();
      }
    }

    // Real-time invitations used by NotificationContext
    match /liveInvitations/{invitationId} {
      // Participants can read invitations sent to them
      // Organizers can read invitations they sent
      ...
      allow create: if isOrganizerOrTeacher() || isAdmin();
      // Participants can update their own invitations (accept/decline)
      // Organizers can update invitations they sent
      ...
      // Only organizers who sent the invitation or admins can delete
      allow delete: if isAuthenticated() && (
    ...
    // Room Code Invitations (keep as provided)
    match /roomCodeInvitations/{invitationId} {
      allow read: if isAuthenticated() && (
        request.auth.email == resource.data.participantEmail ||
