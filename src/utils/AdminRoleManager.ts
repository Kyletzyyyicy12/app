  ...
  static async changeUserRole(request: RoleChangeRequest): Promise<boolean> {
    try {
      ...
      if (!isAuthorized) {
        Alert.alert(
          'Access Denied',
          'Only administrators can change user roles. This incident will be logged.'
        )
        return false
      }
      ...
      Alert.alert(
        ...
        `User ${request.targetUserEmail} role changed from ${oldRole} to ${request.newRole}.`
      )
